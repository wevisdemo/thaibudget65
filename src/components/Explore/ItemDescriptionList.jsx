...
function ItemDescriptionList({
  ...
  const sorter = (sortBy, data) => {
    if (sortBy === 'AMOUNT') {
      return data.sort((a, b) => (parseFloat(b[sortBy].replaceAll(',', '')) - parseFloat(a[sortBy].replaceAll(',', ''))));
    }
  ...
  return (
    <div className="w-full mt-2">
      ...
      <div className="flex flex-col xl:flex-row xl:justify-between items-center my-4">
        ...
        <Pagination currentPage={currentPage} pageLength={Math.ceil(items.length / ITEM_PER_PAGE)} setCurrentPage={setCurrentPage} />
      </div>
      {items && items
